...
async function getMDXContent(locale: string) {
  const filePath = path.join(process.cwd(), "content", "the-forge-race-tier-list", `${locale}.mdx`);
  try {
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "RaceTierList" });
  ...
  return constructMetadata({
    ...
    keywords: [
      "the forge race tier list", "the forge best race", "the forge races",
      "the forge all races", "the forge race guide", "the forge race reroll",
      "the forge roblox races", "the forge archangel", "the forge race odds",
    ],
    locale: locale as Locale,
    path: "/the-forge-race-tier-list",
    canonicalUrl: "/the-forge-race-tier-list",
  });
}

export default async function RaceTierListPage({ params }: { params: Params }) {
  const { locale } = await params;
    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: "Race Tier List", url: `${BASE_URL}/the-forge-race-tier-list` },
        ])}
      />
      <header className="relative overflow-hidden rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50 via-violet-50 to-purple-50 p-6 dark:border-indigo-900/40 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
        ...
        <h1 className="relative font-heading text-3xl font-black text-slate-900 dark:text-slate-100 sm:text-4xl">
          The Forge Race Tier List — All 16 Races Ranked
        </h1>
        <p className="relative mt-4 text-slate-700 dark:text-slate-300">
          Complete tier list ranking every race in The Forge on Roblox by combat
          effectiveness, mining bonuses, and overall value. Includes drop rates,
          traits, and reroll strategy.
        </p>
      </header>
