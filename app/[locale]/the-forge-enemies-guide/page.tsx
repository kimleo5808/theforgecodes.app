...
async function getMDXContent(locale: string) {
  const filePath = path.join(process.cwd(), "content", "the-forge-enemies-guide", `${locale}.mdx`);
  try {
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "EnemiesGuide" });
  ...
  return constructMetadata({
    ...
    keywords: [
      "the forge enemies", "the forge all enemies", "the forge boss guide",
      "the forge combat guide", "the forge enemy list", "the forge roblox enemies",
      "the forge boss strategy",
    ],
    locale: locale as Locale,
    path: "/the-forge-enemies-guide",
    canonicalUrl: "/the-forge-enemies-guide",
  });
}

export default async function EnemiesGuidePage({ params }: { params: Params }) {
  const { locale } = await params;
    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: "Enemies Guide", url: `${BASE_URL}/the-forge-enemies-guide` },
        ])}
      />
      <header className="relative overflow-hidden rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50 via-violet-50 to-purple-50 p-6 dark:border-indigo-900/40 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
        ...
        <h1 className="relative font-heading text-3xl font-black text-slate-900 dark:text-slate-100 sm:text-4xl">
          All Enemies in The Forge — Complete Combat Guide
        </h1>
        <p className="relative mt-4 text-slate-700 dark:text-slate-300">
          Every enemy and boss in The Forge on Roblox listed by region with HP, damage, loot drops, and combat strategies.
        </p>
      </header>
