...
async function getMDXContent(locale: string) {
  const filePath = path.join(process.cwd(), "content", "the-forge-totems-guide", `${locale}.mdx`);
  try {
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "TotemsGuide" });
  ...
  return constructMetadata({
    ...
    keywords: [
      "the forge totems", "the forge all totems", "the forge luck totem",
      "the forge totem guide", "the forge best totem", "the forge roblox totems",
      "the forge totem list",
    ],
    locale: locale as Locale,
    path: "/the-forge-totems-guide",
    canonicalUrl: "/the-forge-totems-guide",
  });
}

export default async function TotemsGuidePage({ params }: { params: Params }) {
  const { locale } = await params;
    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: "Totems Guide", url: `${BASE_URL}/the-forge-totems-guide` },
        ])}
      />
      <header className="relative overflow-hidden rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50 via-violet-50 to-purple-50 p-6 dark:border-indigo-900/40 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
        ...
        <h1 className="relative font-heading text-3xl font-black text-slate-900 dark:text-slate-100 sm:text-4xl">
          All Totems in The Forge — Complete Guide
        </h1>
        <p className="relative mt-4 text-slate-700 dark:text-slate-300">
          Every totem in The Forge on Roblox explained with effects, duration,
          pricing, free totem codes, and which totems to prioritize.
        </p>
      </header>
