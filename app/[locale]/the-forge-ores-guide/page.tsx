...
async function getMDXContent(locale: string) {
  const filePath = path.join(process.cwd(), "content", "the-forge-ores-guide", `${locale}.mdx`);
  try {
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "OresGuide" });
  ...
  return constructMetadata({
    ...
    keywords: [
      "the forge ores", "the forge all ores", "the forge ore list",
      "the forge mining guide", "the forge best ore", "the forge rare ores",
      "the forge roblox ores", "the forge demonite", "the forge arcane crystal",
    ],
    locale: locale as Locale,
    path: "/the-forge-ores-guide",
    canonicalUrl: "/the-forge-ores-guide",
  });
}

export default async function OresGuidePage({ params }: { params: Params }) {
  const { locale } = await params;
    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: "Ores Guide", url: `${BASE_URL}/the-forge-ores-guide` },
        ])}
      />
      <header className="relative overflow-hidden rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50 via-violet-50 to-purple-50 p-6 dark:border-indigo-900/40 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
        ...
        <h1 className="relative font-heading text-3xl font-black text-slate-900 dark:text-slate-100 sm:text-4xl">
          All Ores in The Forge — Complete Mining Guide
        </h1>
        <p className="relative mt-4 text-slate-700 dark:text-slate-300">
          Every ore in The Forge on Roblox listed by region and rarity, with
          mining tips, rare ore locations, and crafting uses explained.
        </p>
      </header>
