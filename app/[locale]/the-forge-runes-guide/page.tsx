...
async function getMDXContent(locale: string) {
  const filePath = path.join(process.cwd(), "content", "the-forge-runes-guide", `${locale}.mdx`);
  try {
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "RunesGuide" });
  ...
  return constructMetadata({
    ...
    keywords: [
      "the forge runes", "the forge rune guide", "the forge all runes",
      "the forge best runes", "the forge rune builds", "the forge roblox runes",
      "the forge rune list",
    ],
    locale: locale as Locale,
    path: "/the-forge-runes-guide",
    canonicalUrl: "/the-forge-runes-guide",
  });
}

export default async function RunesGuidePage({ params }: { params: Params }) {
  const { locale } = await params;
    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: "Runes Guide", url: `${BASE_URL}/the-forge-runes-guide` },
        ])}
      />
      <header className="relative overflow-hidden rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50 via-violet-50 to-purple-50 p-6 dark:border-indigo-900/40 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
        ...
        <h1 className="relative font-heading text-3xl font-black text-slate-900 dark:text-slate-100 sm:text-4xl">
          The Forge Runes Guide — All Runes & Best Builds
        </h1>
        <p className="relative mt-4 text-slate-700 dark:text-slate-300">
          Complete rune guide for The Forge on Roblox with all runes listed, best build combinations for mining, combat, and hybrid playstyles.
        </p>
      </header>
