...
async function getMDXContent(locale: string) {
  const filePath = path.join(process.cwd(), "content", "the-forge-pickaxes-guide", `${locale}.mdx`);
  try {
...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
  ...
  const t = await getTranslations({ locale, namespace: "PickaxesGuide" });
  ...
  return constructMetadata({
    ...
    keywords: [
      "the forge pickaxes", "the forge all pickaxes", "the forge best pickaxe",
      "the forge pickaxe guide", "the forge pickaxe list", "the forge pickaxe tier list",
      "the forge roblox pickaxes", "the forge dragon head pickaxe", "the forge prismatic pickaxe",
    ],
    locale: locale as Locale,
    path: "/the-forge-pickaxes-guide",
    canonicalUrl: "/the-forge-pickaxes-guide",
  });
}

export default async function PickaxesGuidePage({ params }: { params: Params }) {
  const { locale } = await params;
    ...
    <div className="mx-auto w-full max-w-4xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
      <JsonLd
        data={breadcrumbSchema([
          ...
          { name: "Pickaxes Guide", url: `${BASE_URL}/the-forge-pickaxes-guide` },
        ])}
      />
      <header className="relative overflow-hidden rounded-2xl border border-indigo-200/70 bg-gradient-to-br from-indigo-50 via-violet-50 to-purple-50 p-6 dark:border-indigo-900/40 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
        ...
        <h1 className="relative font-heading text-3xl font-black text-slate-900 dark:text-slate-100 sm:text-4xl">
          All Pickaxes in The Forge — Guide & Tier List
        </h1>
        <p className="relative mt-4 text-slate-700 dark:text-slate-300">
          Every pickaxe in The Forge on Roblox ranked with full stats, prices,
          upgrade paths, and hidden pickaxe locations.
        </p>
      </header>
